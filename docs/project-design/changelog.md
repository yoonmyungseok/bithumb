...
- **매수 종목 선정·진입 타점·청산 타이밍 손익비 정상화 (상투 매수 및 푼돈 익절 근절)**:
  - `src/strategy_engine.py`:
    - **호가 잔량비(Orderbook Ratio) 하드게이트 일원화**: `entry_signal`에 `hard_gate_orderbook`을 추가하여, 매수벽 대비 매도벽이 두터운 종목(`orderbook_smoothed_ratio < 1.0`, 약세장 기준)은 AI 및 로컬 룰 승인 여부와 무관하게 신규 BUY를 Fail-Closed로 전면 차단 (FLOCK, PUMP 등 매도벽 잡알트 선정 방지).
    - **전고점 상투(꼭지) 매수 원천 차단**: 직전 12개 봉(60분) 최고점 턱밑(거리 0.0%~0.3%) 추격매수 예외(알파 점수 55점 이상 시 면제)를 전면 삭제하고, 최소 0.6%(주도주 0.3%) 이상 눌림목 지지가 형성된 자리에서만 진입을 허용(`PULLBACK_MIN_DISTANCE_BELOW_HIGH = 0.006`).
    - **약세장(`RISK_OFF`) 모멘텀 돌파 전면 금지**: 비트코인이 약세/조정 국면일 때는 알트코인의 돌파 신호가 대다수 페이크 돌파(Bull Trap) 및 윗꼬리 급락으로 이어지므로, `RISK_OFF`에서는 모멘텀 돌파(`MOMENTUM_BREAKOUT`) 신규 진입을 전면 차단하고 바닥 지지 반등(`PULLBACK`) 및 개미털기 역매수(`SHAKEOUT_SWEEP`)만 허용.
    - **볼린저 밴드 과열 상한 정상화**: %B 상한을 일반장 `0.70`(`PCT_B_MAX`), 약세장 `0.65`(`PCT_B_MAX_RISK_OFF`), RSI 상한을 일반장 68.0, 약세장 62.0으로 정상화하여 꼭대기 매수를 원천 차단.
    - **1차 분할익절 비중 상향**: `PARTIAL_TP_1_RATIO`를 기존 40%에서 **60%**로 상향하여 1차 목표 달성 시 의미 있는 수익금을 확정.
  - `src/trading_runtime.py`:
    - **공통 안전 가드 적용**: 로컬 룰 승인(`selected_entry.get("allow_buy")`) 경로에도 상투 과열 차단(%B 한도), 약세장 반등 확정 여부, 호가 잔량비 검사를 일원화하여 필수 적용.
    - **타임스탑 푼돈 익절 폐지 및 본전 보장(Auto Break-Even) 홀딩 전환**: 120분 경과 시 손익률이 실질 본전(+0.3% 이상)인 경우 푼돈(300원~400원) 시장가 투매를 전면 폐지하고, 즉시 본전 보장 스탑(`Auto Break-Even`)을 락인한 후 목표가(+4.0%) 또는 트레일링 스탑까지 끝까지 추세를 추종하도록 전환.
  - `src/risk_manager.py`:
    - `activate_breakeven(market)` 메서드를 추가하여 횡보 타임스탑 시 수익권 포지션의 본전 보장 스탑을 안전하게 원자적으로 강제 활성화.
  - `src/market_screener.py`:
    - 모멘텀 주도주 스크리닝 요건을 당일 변동률 3.0% 이상 및 상대강도(RS) 1.5% 이상으로 정상화하여 잡알트의 무분별한 모멘텀 돌파 분류 방어.
...
- **Gemini API 구글 콘솔(Google AI Studio) RPD 기준 대시보드 표시 정밀화**:
  - `dashboard/src/app.js`:
    ...
    - 모델별 상세 목록에서도 성공 건수(`modelSuccess`)를 주 지표로 소진율(%)을 산출하고, 503/타임아웃 등 실패 시도(`modelErrors`)를 별도 병기하여 구글 콘솔 화면과의 1:1 대조 정밀성 확보.
  - `dashboard/index.html`:
    - 빗썸 및 업비트 Gemini AI 카드의 사용량 라벨을 `일일 유효 사용량 (구글 콘솔 RPD)`로 명확화.
  - `docs/project-design/operations-and-observability.md`:
    - 구글 콘솔 RPD(성공 유효 호출)와 봇 내부 안전 시도량(총 HTTP 시도)의 대시보드 표기 기준 동기화 명시.
