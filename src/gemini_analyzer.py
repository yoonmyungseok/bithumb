    ...
    def analyze(
        ...
        if normalized_candidate_type == "SWING":
            ...
            policy_details = (
                ...
                f"(EXTENDED 단계는 로컬 퀀트 통과 및 AI 알파 {StrategyPolicy.MOMENTUM_EXTENDED_ALPHA_THRESHOLD_RISK_OFF}점 이상"
...
2. 각 충족 판정에는 이 요청에 포함된 수치 또는 상태를 하나씩 근거로 사용하세요.
3. 후보 유형과 정책 경로에 맞는 경로 조건을 확인하세요. `MOMENTUM_BREAKOUT`은 돌파 추격을 일반 눌림목 근거로, `RECOVERY_REBOUND`는 반등 기대를 일반 상승장 근거로, `NEW_LISTING`은 신규상장 초기 수급을 일반 확인형·모멘텀·반등 근거로 바꾸어 해석하지 마세요.
4. `ALPHA_SCORE`는 위 7대 팩터의 검증 결과만 반영한 0~100 정수입니다. BUY는 현재 알파 승인 기준 이상이고, 핵심 안전 조건(손익비 확보 및 과열 미발생)이 충족될 때 선택하세요.
5. 결론을 내리기 전에 가장 강한 반대 근거 1개와 매수 가설의 무효화 조건 1개를 확인하세요. 단, 손절선과 손익비(1:1.3 이상)가 명확히 확보되어 리스크 관리가 가능한 통제된 위험인 경우 과도하게 방어적인 HOLD를 지양하고 BUY를 승인하세요.
...
1. "EMERGENCY_EXIT": 거래량 실린 윗꼬리 장대음봉, 대량 매도벽 출회, VWAP 이탈, 직전 저점 붕괴 등 돌이킬 수 없는 세력 덤핑/급락 징후 시 기계적 손절 전에 "즉시 시장가 전량 탈출" (※ 주의: 진입 초기 10분 이내의 단순 호가 흔들림, 손실률 -0.8% 이내의 미세 조정, 지지선 유지 상태에서는 절대 남발하지 말고 HOLD 선택)
2. "RUNNER_HOLD": 거래량 급증 장대양봉 돌파, 신고가 랠리 강력 지속 시 "기계적 소액 익절을 보류하고 목표가를 상향하여 추세 최대 향유"
3. "TIGHTEN_STOP": 현재 손익률이 최소 +2.0% 이상 확보된 안정적 수익권에서만 선택 가능하며, 수익 보전을 위해 손절가를 평단가 이상(안전 마진 확보)으로 상향 (※ 주의: 손익률 +2.0% 미만의 보합/미세수익 구간에서는 호가 노이즈 조기 털림 방지를 위해 절대 TIGHTEN_STOP을 선택하지 말고 HOLD 유지)
...
- TIGHTEN_STOP은 반드시 현재 손익률이 +2.0% 이상일 때만 선택하십시오. +0.1%~+1.5% 수준의 미미한 수익 상태에서 조급하게 손절선을 평단가로 바짝 붙이면 정상적인 호가 흔들림에 털려 추세를 놓치게 되므로, 이 구간의 건강한 숨고르기는 반드시 HOLD로 대응하십시오.
...
{{
  ...
  "ADJUSTED_STOP_LOSS": {round(avg_buy_price * 1.015, 2)},
  "CONFIDENCE": 85
}}
"""
