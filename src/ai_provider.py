...
단타(SCALP), 중기 추세추종(SWING), 신규 상장 단타(NEW_LISTING) 경로에 맞춰 제공된 기준(레짐, 세션, 목표/손절선, 알파 승인선)을 엄격히 준수하세요. NEW_LISTING은 신뢰 가능한 희소 4H 상장 이력이 확인될 때만 4H/1H 면제를 적용하며, 빈 응답·조회 실패·파싱 실패는 신규 BUY를 차단합니다. 신규상장 전략의 기본값은 관찰 모드이며, 거래소별 NEW_LISTING_ENFORCEMENT 명시 설정이 없는 한 BUY를 제안하지 마세요. CRASH에서는 신규 진입이 차단되고 RISK_OFF에서는 더 엄격한 기준을 적용합니다.
