...
class BaseGeminiProvider:
    ...
    _COOLDOWN_REASONS_BY_EXCHANGE: ClassVar[dict[str, dict[str, str]]] = {}
    ...
    def clear_cooldowns(cls, exchange: str | None = None) -> None:
        ...
        with cls._COOLDOWN_LOCK:
            if exchange:
                ...
                cls._COOLDOWN_REASONS_BY_EXCHANGE.pop(exchange, None)
            else:
                cls._COOLDOWNS_BY_EXCHANGE.clear()
                cls._COOLDOWN_REASONS_BY_EXCHANGE.clear()
    ...
    def clear_model_cooldown(self, model: str) -> None:
        """지정된 모델의 쿨다운을 거래소 캐시 및 GeminiAnalyzer에서 해제합니다."""
        with self._COOLDOWN_LOCK:
            exchange_map = self._COOLDOWNS_BY_EXCHANGE.setdefault(self.exchange, {})
            exchange_map.pop(model, None)
            reason_map = self._COOLDOWN_REASONS_BY_EXCHANGE.setdefault(self.exchange, {})
        ...
        try:
            from gemini_analyzer import GeminiAnalyzer
            with GeminiAnalyzer._CLASS_LOCK:
                GeminiAnalyzer._MODEL_COOLDOWNS.pop(model, None)
        except Exception:
            pass
    ...
    def can_call_model_safety(self, model: str, for_emergency_exit: bool = False) -> bool:
        ...
        - 업비트: GeminiTelemetry.can_call_model 검증 (소진 시 PT 자정까지 쿨다운 자동 등록, 쿼터 여유 시 사전 차단 쿨다운 자동 해제)
...
현재 레짐, 후보 경로(SCALP, SWING, MOMENTUM_BREAKOUT, RECOVERY_REBOUND, NEW_LISTING, SHAKEOUT_SWEEP), RS 주도주(RS >= +2.0%) 특례 규정, 신규상장 정책과 안전 차단 조건을 준수하세요. 상투·고점 추격 매수(Chasing the Top)는 절대 금지하며, BUY 승인 시 진입가(ENTRY_PRICE)는 현재가 추격을 지양하고 5분봉 VWAP, MA20 또는 전저점 지지선 부근의 저점 눌림목 지정가(현재가 이하)로 산출하여 안전마진을 확보하세요. MOMENTUM_BREAKOUT의 EXTENDED는 확정 5분봉 고점 돌파·양봉·거래량·RSI·1시간 EMA20을 모두 통과한 경우에만 주간 알파 55점/심야 65점 이상으로 제한 추격 BUY를 검토하며, 최초 비중은 최대 종목 비중의 15%를 넘지 않습니다. 알트코인은 비트코인 급락(CRASH)이 아닌 이상 비트코인의 추세(조정·약세·횡보)에 영향없이 자체 기술적 지표와 수급(체결강도 75% 이상, 호가 갭 0.50% 이하)이 양호하고 손익비(1:1.3 이상)가 확보되면 1시간봉 조정 구간이더라도 5분봉 지지선 안착을 확인하여 독립적으로 매수를 승인하되, 비트코인 대폭락(CRASH)에서는 신규 진입을 제안하지 마세요. 스크리너 단계 신규상장 사전 필터로 `classify_listing_maturity()` + `is_new_listing_eligible()` SSOT에 따라 자격 미충족 NEW_LISTING 후보는 AI 입력 전에 제외됩니다.
...
ACK는 체결이 아닙니다. REST 또는 Private WebSocket의 확정 체결 전에는 포지션·손익·쿨다운·주문 완료를 단정하지 마세요.
불확실하거나 데이터가 누락·모순되면 BUY가 아닌 HOLD를 선택하세요. 주문 실행·취소·체결 확정 권한은 없습니다.
현재 레짐, 후보 경로(SCALP, SWING, MOMENTUM_BREAKOUT, RECOVERY_REBOUND, NEW_LISTING, SHAKEOUT_SWEEP), RS 주도주(RS >= +2.0%) 특례 규정, 신규상장 정책과 안전 차단 조건을 준수하세요. 상투·고점 추격 매수(Chasing the Top)는 절대 금지하며, BUY 승인 시 진입가(ENTRY_PRICE)는 현재가 추격을 지양하고 5분봉 VWAP, MA20 또는 전저점 지지선 부근의 저점 눌림목 지정가(현재가 이하)로 산출하여 안전마진을 확보하세요. 1시간봉 EMA20 대비 과도하게 이탈한(약세장 -1.0% 초과) 역추세 하락 종목은 배제하고, 단가 10원 미만 초저가 코인은 수수료·슬리피지 방어를 위한 실질 BEP 완충 마진(+0.55% 이상)을 확인하세요. MOMENTUM_BREAKOUT의 EXTENDED는 확정 5분봉 고점 돌파·양봉·거래량·RSI·1시간 EMA20을 모두 통과한 경우에만 주간 알파 55점/심야 65점 이상으로 제한 추격 BUY를 검토하며, 최초 비중은 최대 종목 비중의 15%를 넘지 않습니다. 알트코인은 비트코인 급락(CRASH)이 아닌 이상 비트코인의 추세(조정·약세·횡보)에 영향없이 자체 기술적 지표와 수급(체결강도 75% 이상, 호가 갭 0.50% 이하)이 양호하고 손익비(1:1.3 이상)가 확보되면 1시간봉 조정 구간이더라도 5분봉 지지선 안착을 확인하여 독립적으로 매수를 승인하되, 비트코인 대폭락(CRASH)에서는 신규 진입을 제안하지 마세요. 스크리너 단계 신규상장 사전 필터로 `classify_listing_maturity()` + `is_new_listing_eligible()` SSOT에 따라 자격 미충족 NEW_LISTING 후보는 AI 입력 전에 제외됩니다.
API 키, 시크릿, 토큰, 계좌 또는 주문 식별자를 요구·출력·재현하지 마세요. JSON 요청에는 마크다운 없는 유효 JSON만 반환하고, 모든 설명 텍스트는 자연스러운 한국어로만 작성하세요."""
