...
def _get_bithumb_portfolio_tiers(total_equity: float) -> tuple[int, float, int]:
    """런타임 동적 설정(MAX_OPEN_POSITIONS, MAX_POSITION_PCT)을 최우선 존중하는 자산 티어 도출"""
...
cycle_engine = TradingCycleEngine(
    ...
    TradingRuntimeContext(
        ...
        get_portfolio_tiers=_get_bithumb_portfolio_tiers,
        order_executor=order_executor,
...
def send_daily_morning_report():
    ...
    execute_daily_morning_report_shared(
        ...
        risk_manager=risk_manager,
        telegram=telegram,
        build_analyzer=build_bithumb_analyzer,
        logger=logger,
        web_port=WEB_PORT,
    )
