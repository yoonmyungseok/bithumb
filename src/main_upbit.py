...
MAX_SWING_POSITIONS = min(
    MAX_OPEN_POSITIONS,
    max(0, int(os.getenv("UPBIT_MAX_SWING_POSITIONS", os.getenv("MAX_SWING_POSITIONS", "1")))),
)
MAX_NEW_LISTING_POSITIONS = min(
    ...
    max(
        ...
        int(
            os.getenv(
                "UPBIT_MAX_NEW_LISTING_POSITIONS",
...
risk_guard = RiskGuard(
    ...
    dynamic_slots_enabled=StrategyPolicy.is_dynamic_slots_enabled("upbit"),
)
    ...
    private_ws = UpbitPrivateWebSocketClient(
        ...
        on_queue_overflow=lambda: (
            ...
            telegram.send_debounced_message(
                category_key="upbit_private_ws_queue_full",
                text=(
                    "⚠️ <b>[업비트 Private WebSocket 주문 이벤트 큐 포화]</b>\n"
...
def _reconcile_after_private_ws_drain() -> None:
    ...
    cycle_orchestrator.reconcile_orders(
        exchange, order_journal, fill_processor, label="업비트 ",
    )
...
UPBIT_CYCLE_PROFILE = ExchangeCycleProfile(
    ...
    max_cycle_markets=UPBIT_MAX_CYCLE_MARKETS,
)
