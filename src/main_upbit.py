...
def send_daily_morning_report():
    """매일 아침 09:00 KST 업비트 일일 결산 모닝 리포트 전송 및 구글 스프레드시트 일괄 배치 동기화"""
    now_str = get_kst_now_str()
    logger.info(f"📊 [업비트 아침 9시 일일 결산 브리핑 발송 및 구글 시트 일괄 동기화: {now_str}]")
    ...
    try:
        ...
        # 🔄 업비트 구글 스프레드시트 09:00 일괄 배치 동기화 실행
        ...
        if sheets:
            try:
                ...
                synced = sheets.sync_all_daily_batch(
                    ...
                    get_korean_name_fn=upbit.get_korean_name,
                )
