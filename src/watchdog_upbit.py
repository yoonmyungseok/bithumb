...
file_handler.setFormatter(
    logging.Formatter(
        "[%(asctime)s] [%(levelname)s] [UPBIT-WATCHDOG] %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S",
    )
)
...
UPBIT_WATCHDOG_PROFILE = ExchangeWatchdogProfile(
    exchange_key="upbit",
    data_dir=DATA_DIR,
    main_script_name="main_upbit.py",
    startup_banner_lines=(
        "  Upbit Bot Smart Watchdog Engine 가동 시작",
        "  (24시간 무중단 감시 및 자동 복구 시스템)",
    ),
    duplicate_instance_warning="⚠️ 이미 실행 중인 업비트 워치독 인스턴스가 존재합니다. 중복 실행을 방지하고 종료합니다.",
    shutdown_signal_message="🛑 업비트 워치독 종료 시그널 감지. 봇 프로세스를 안전하게 종료합니다.",
    shutdown_complete_message="✅ 업비트 봇 및 워치독 안전 종료 완료.",
    process_start_log_label="🚀 [업비트 봇 프로세스 시작]",
    process_spawn_error_log="업비트 봇 프로세스 실행 실패",
    hang_detect_log_label="Upbit Hang 감지",
    abnormal_exit_log_label="⚠️ [업비트 봇 비정상 종료 감지]",
    crash_loop_alert_title="업비트 봇 긴급 알림 - 연속 크래시 감지",
    crash_recovery_alert_title="업비트 봇 비정상 종료 감지 & 자동 복구",
    crash_recovery_process_line="업비트 봇 프로세스가 예기치 않게 종료되었습니다.",
    crash_restart_label="업비트 봇",
)


def main():
    watchdog = TradingBotWatchdog(
        UPBIT_WATCHDOG_PROFILE,
