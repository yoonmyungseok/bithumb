...
- **급락 후 반등 전용 진입**: 회복 구간에서 일반 `RISK_OFF` 신호가 미달한 후보는 `recovery_rebound_signal()`로 한 번 더 평가합니다. BTC `CRASH`·주문 대사 미완료·시세 스트림 불안정·종목별 쿨다운은 절대 우회하지 않습니다. 확정 5분봉 반등, 핵심 하드게이트, 알파 75점 이상, BTC 대비 RS +1.5% 이상, 최소 거래대금(10억 원, `MIN_TRADE_VALUE_RISK_OFF`), EMA20 대비 1% 이내의 1시간봉 회복을 모두 만족할 때만 양 거래소에서 실제 주문을 허용합니다. 반등 주문은 일반 슬롯의 35%를 기본 비중으로 하며, 동일 회복 구간에는 한 주문만 제출합니다.
...
| **v8.47** | 2026-09-08 | • **신규 상장 단타(NEW_LISTING) RISK_OFF 진입 허용 및 엄격 기준 연동**<br>• **`is_new_listing_eligible()`·`new_listing_entry_signal()`**: `RISK_OFF` 전면 차단을 제거하고 `CRASH`만 신규 진입 차단. `RISK_OFF`에서는 거래대금 30억·당일 상승률 2.0%~6.0%·RS +1.5% 하드게이트 적용<br>• **`get_new_listing_alpha_threshold()`**: `RISK_OFF` 80점·심야 `RISK_OFF` 85점 기준 추가<br>• **AI 프롬프트**: `GeminiAnalyzer`·`GroqProvider.SYSTEM_INSTRUCTION`에 `RISK_OFF` 엄격 기준 문구 반영, `CRASH` 차단만 유지<br>• **검증**: `tests/test_new_listing_gate.py`, `tests/test_gemini_prompt_contract.py`, `tests/test_bithumb_groq_provider.py` 회귀 테스트 갱신 |
| **v8.46** | 2026-09-08 | • **신규 상장 단타(NEW_LISTING) 운영 슬롯·쿨다운·대시보드 모니터링 (PR5)**<br>• **`RiskGuard` Triple-Track**: `max_new_listing_positions`·`held_new_listing_markets` 추가, `SWING`/`NEW_LISTING`/`SCALP` 슬롯 독립 검증. 빗썸 env `BITHUMB_MAX_NEW_LISTING_POSITIONS`·업비트 `UPBIT_MAX_NEW_LISTING_POSITIONS`(공통 `MAX_NEW_LISTING_POSITIONS` 폴백) 연동<br>• **`trading_runtime.process_buy_execution()`**: `get_new_listing_markets()` 전달로 신규상장 1건 제한 보장<br>• **`CooldownManager`**: `NEW_LISTING_TIME_STOP`/`NEW_LISTING_EARLY_EXIT` 청산 후 `NEW_LISTING_REENTRY_COOLDOWN_SEC`(3600초) 재진입 쿨다운. `fill_processor`는 원본 `exit_reason` 코드를 쿨다운에 전달<br>• **대시보드·진단**: `bot_controller` policy/diag/dashboard API에 신규상장 슬롯·관찰 모드·`strategy_mode`/`candidate_type`/`listing_maturity` 노출, `dashboard_server` combined 슬롯 합산, 안전 패널·정책 패널 UI<br>• **검증**: `tests/test_exchange_slot_isolation.py`, `tests/test_new_listing_gate.py` 슬롯·쿨다운 회귀 테스트 갱신 |
| **v8.45** | 2026-09-08 | • **신규 상장 단타(NEW_LISTING) AI 프롬프트·대시보드 한국어 라벨 연동 (PR4)**<br>• **`GeminiAnalyzer` NEW_LISTING 분기**: `candidate_type=NEW_LISTING` 시 4H/1H 면제·상장 72시간·거래대금 20억/30억·당일 상승률 1.5%~8.0%·RS +1.0%·알파 75/80·비중 15%·목표/손절·청산 규칙을 `get_new_listing_alpha_threshold()`와 `StrategyPolicy` SSOT로 프롬프트에 주입. `MOMENTUM_BREAKOUT`/`RECOVERY_REBOUND`/`CONFIRMED` 근거 대체 금지·RISK_OFF/CRASH 차단 문구 포함<br>• **`GroqProvider.SYSTEM_INSTRUCTION`**: SCALP/SWING/NEW_LISTING 세 경로 명시 및 RISK_OFF/CRASH 신규 진입 차단 원칙 반영<br>• **대시보드 표시 전용 한글 라벨**: `NEW_LISTING`·`NEW_LISTING_TIME_STOP`·`NEW_LISTING_EARLY_EXIT`·`SWING_TREND_STOP`·`strategy_mode` 매핑 (`dashboard/src/app.js`, 레거시 `web_server.py`)<br>• **검증**: `tests/test_gemini_prompt_contract.py`, `tests/test_bithumb_groq_provider.py`, `tests/test_dashboard_frontend_korean.py`, `tests/test_trade_improvement_guards.py` 회귀 테스트 추가 |
| **v8.44** | 2026-09-08 | • **신규 상장 단타(NEW_LISTING) 전용 청산 규칙 연동 (PR3)**<br>• **`TrailingStopTracker` 신규상장 모드**: `is_new_listing_position()`·`get_new_listing_markets()` 추가, 분할익절 +3.0%(50%)·트레일링 +4.0%/드롭 2.0%만 적용하고 2차 분할익절은 비활성화<br>• **`process_priority_exits()`·`RealtimeRiskEngine` 연동**: 신규상장 포지션은 하드스탑 -4.0%·손절 -2.5%·60분 타임스탑·30분 조기탈출(-0.5%~+0.3%)을 사용하고 스윙 4H 추세청산·일반 120분 타임스탑·모멘텀 조기탈출은 면제. 첫 매수 체결 시 `strategy_mode=NEW_LISTING` 확정은 기존 `OrderFillProcessor` 경계 유지<br>• **검증**: `tests/test_new_listing_gate.py` 청산·타임스탑 회귀 테스트 추가 |
| **v8.43** | 2026-09-08 | • **신규 상장 단타(NEW_LISTING) 런타임 진입 게이트 연동 (PR2)**<br>• **`process_entry_gating()` 분기 재구성**: `classify_listing_maturity()`가 `NEW_LISTING`이면 4H·1H 확정봉 게이트를 면제하고 5분 확정 5개·`is_new_listing_eligible()`·`new_listing_entry_signal()`로만 1차 퀀트를 평가. `MATURE` 경로는 기존 4H/5m/1h 게이트 유지<br>• **AI BUY 필수·관찰 모드**: 신규상장 최종 BUY는 AI 호출·`BUY` 확인이 필요하고 `NEW_LISTING_ENFORCEMENT=false`면 audit만 수행. 사이클당 1건 제한은 모멘텀과 공유<br>• **검증**: `tests/test_trading_runtime.py`, `tests/test_new_listing_gate.py` 회귀 테스트 추가 |
| **v8.42** | 2026-09-08 | • **신규 상장 단타(NEW_LISTING) 정책 SSOT 1차 도입 (PR1)**<br>• **`StrategyPolicy` 신규상장 전용 상수**: 5분 확정봉 최소 5개·상장 윈도우 72시간·비중 15%·손절 -2.5%·하드스탑 -4%·60분 타임스탑·30분 조기탈출 등 단타 전용 파라미터를 `strategy_engine.py`에 일원화<br>• **판정 SSOT**: `classify_listing_maturity()`가 `MATURE`/`NEW_LISTING`/`INSUFFICIENT`를 반환하고, `is_new_listing_eligible()`가 거래대금·RS·당일 상승률·BTC `CRASH`/`RISK_OFF` 차단을 검증. 런타임 연동(PR2)·청산(PR3)은 후속 PR에서 적용<br>• **검증**: `tests/test_new_listing_gate.py` 신규 작성 |
| **v8.39** | 2026-09-08 | • **10원 미만 초저가 코인 제한 전면 해제 & 소수점 호가 정밀도 지원 (`MIN_ASSET_PRICE_KRW=0.0001`)**<br>• **초저가 코인 진입 개방**: 기존 `MIN_ASSET_PRICE_KRW = 10.0`으로 원천 차단되던 10원 미만 급등 코인(SOPH 9.5원, MANTRA 7.3원, QKC 3.3원, OBSR 0.91원 등)의 제한을 전면 해제하여 거래소 최소 호가 단위(0.0001원)까지 정상 스크리닝 및 매수 허용<br>• **소수점 목표가/손절선 정밀도 확장**: 1원 미만 코인의 익절/손절 계산 시 2자리 고정 반올림으로 인한 가격 왜곡을 방지하고 소수점 4자리까지 유연하게 보존(`round(price, 4)`)<br>• **호가 안전망 불변**: 매수/매도 스프레드 0.35% 이하 및 상위 5호가 누적 매수 잔량 2,000만 원 이상 검증은 fail-closed로 유지하여 유동성 없는 위험 호가는 자동 방어 |
| **v8.38** | 2026-09-08 | • **AI 심층 분석 우선순위 큐 정렬 & 사이클당 후보 예산 확대 (`MAX_AI_CANDIDATES_PER_CYCLE=4`)**<br>• **로컬 퀀트 알파 기반 AI 우선순위 큐 정렬**: 스크리너 단순 순위로 인한 AI 예산 조기 소진(체결강도 불량 종목의 쿼터 낭비)을 방지하기 위해, 사이클 내 모든 후보를 로컬 퀀트 사전 평가(Tier 1: allow_buy 즉시 매수 적격 & 고알파, Tier 2: 하드게이트 통과 유망 후보, Tier 3: 기타)를 거쳐 재정렬하여 BCH(85점), XRP(75점) 등 최고 우량 종목이 1순위로 AI 심층 분석을 받도록 보장<br>• **사이클당 AI 후보 예산 4개 확대**: 일일 쿼터(Gemini 1,000 RPD, Groq 14,400 RPD) 및 5분봉 동일 캔들 캐시 안전성을 기반으로 `MAX_AI_CANDIDATES_PER_CYCLE` 기본값을 2에서 4로 상향하여 우량 종목 분석 기회 2배 확대<br>• **스냅샷 캐시 탑재**: 우선순위 사전 평가 시 조회한 마켓 스냅샷을 런타임 루프에 전달하여 중복 네트워크 I/O 제거 및 사이클 속도 보존 |
| **v8.37** | 2026-09-08 | • **P0 체결 전 포지션 완료 처리 차단, 스윙 분할익절·4시간봉 추세 청산·운영 슬롯 격리 정합화**<br>• **확정 체결 단일 경계**: `check_position()`의 분할익절·트레일링 신호는 더 이상 `partial_tp_done` 또는 포지션 상태를 변경하지 않으며, `OrderFillProcessor`가 REST/Private WebSocket으로 확인한 `fill_delta > 0`에서만 분할익절 단계와 브레이크이븐을 활성화<br>• **전량 청산 상태 보존**: 실시간/5분 루프 손절, 트레일링, 긴급 매도는 주문 제출·ACK·OPEN·UNKNOWN·거절·부분 체결에서 트레일링·전략 모드를 삭제하지 않고, 부분익절을 제외한 `FILLED` 전량 청산 확인에서만 정리<br>• **스윙 원보유 수량 기준**: 거래소별 저널의 최근 확정 매수 누적 수량을 기준으로 1차 40%, 2차 30%를 계산하며, 1차 부분 체결 뒤에도 2차를 잔여 비율로 왜곡하지 않는다. 원보유 수량 대사 실패 시 분할익절 주문은 추정 제출하지 않고 보류하며, 단타의 거래소별 잔여 비율은 유지<br>• **스윙 4H EMA20 보호**: 시장 스냅샷이 4시간봉 25개를 별도 조회하고 진행 중 봉을 제외한 확정 20개로만 추세 이탈을 판정한다. 4H 조회 실패·부족 시 해당 신규 BUY는 차단하되 하드스탑·확정 손절·체결 대사는 유지하며, 데이터 불능이 12시간 지속될 때만 무기한 보유 방지 보호 청산을 허용<br>• **운영 슬롯 명시 연결**: 빗썸 `BITHUMB_MAX_SWING_POSITIONS`와 업비트 `UPBIT_MAX_SWING_POSITIONS`(각 기본 1)를 실제 `RiskGuard`에 전달한다. 기본적으로 빗썸은 총 3개 중 스윙 1·단타 2, 업비트는 총 2개 중 스윙 1·단타 1을 예약하며, 종목당·총 투자 비중 한도는 슬롯 여유와 별개로 계속 우선 검증<br>• **전략 모드 확정 시점**: 스윙/단타 모드는 신규 매수 주문 제출이 아니라 첫 실제 매수 체결 이후에 설정하여 미체결 주문이 운영 슬롯을 점유하지 않도록 변경<br>• **검증**: `test_confirmed_exit_state.py`, `test_swing_partial_take_profit_sizing.py`, `test_swing_4h_safety.py`, `test_exchange_slot_isolation.py`에서 체결 상태 보존, 양 거래소의 40%/30% 수량, 4H 확정봉·데이터 불능, 거래소별 슬롯·노출 경계를 회귀 검증 |
| **v8.36** | 2026-09-08 | • **대시보드 브라우저 탭 파비콘 및 UI 탭 전용 SVG 벡터 아이콘 적용**<br>• **브라우저 탭 파비콘 제작 및 탑재**: 다크 테마 기반 퀀트 트레이딩 캔들스틱 및 알파 모멘텀 돌파를 상징하는 전용 벡터 파비콘(`dashboard/favicon.svg`) 신규 제작, SPA 및 내장 HTML에 `<link rel="icon">` 탑재 및 정적 서빙(`/favicon.ico`, `/favicon.svg`) 지원<br>• **대시보드 UI 탭 버튼 전용 SVG 아이콘 적용**: 거래소 스위처 탭(전체 통합, 빗썸, 업비트) 및 전략 뷰 스위처 탭(전체 뷰, 보유 포지션, 신규 스캔 후보)에 고유 모티브(글로벌 네트워크, 비트코인/빗썸 코인, 업비트 볼트, 레이아웃 그리드, 캔들스틱, 레이더 타겟)의 세련된 인라인 SVG 아이콘 완비<br>• **단위 및 회귀 검증**: `test_dashboard_server.py`에 파비콘 파일 및 HTML/템플릿 탭 아이콘 검증 추가, 전체 단위/회귀 테스트 100% 통과 |
| **v8.35** | 2026-09-08 | • **상승 알트코인 매수 기회 확보를 위한 퀀트 및 모멘텀 진입 게이트 현실화**<br>• **모멘텀 초입(`EARLY`) 상한선 상향**: `StrategyPolicy.MOMENTUM_EARLY_MAX_CHANGE_RATE`를 기존 3.0%에서 **6.0%**로 현실화하고 환경 변수 연동 헬퍼 `get_momentum_early_max_change_rate()` 도입 (3~6%대 주도주 파동의 초입 정상 진입 보장)<br>• **모멘텀 돌파(`MOMENTUM_BREAKOUT`) 하드게이트 완화**: 5분봉 RSI 상한 완화(72.0 ➜ 78.0), 5분봉 거래량 배수 완화(1.3배 ➜ 1.1배), 1시간봉 MTF 지지 기준 완화(0.990 ➜ 0.980), 모멘텀 돌파 전용 MA20 이격도 상한 `MAX_MA20_DISPARITY_MOMENTUM = 1.050`(+5.0%) 신설로 급등 돌파 캔들 이격 과열 차단 해소<br>• **BTC 약세(`RISK_OFF`) 시 알트 독자 수급 주도주 완화 룰**: BTC 대비 상대강도가 매우 강한 주도주(`RS >= +2.0%`)는 `RISK_OFF` 레짐에서도 1H MTF(0.980) 및 RSI 상한(70.0)을 일반장 기준으로 탄력 적용<br>• **1차 퀀트 반등(`CONFIRMED`) 게이트 현실화**: 약세장 저점 대비 최대 허용 거리(3.5% ➜ 4.5%), 볼린저 `%B` 상한(0.75 ➜ 0.80), RSI 상한(65.0 ➜ 70.0)으로 현실화<br>• **검증**: `test_market_screener.py`, `test_strategy_engine.py`, `test_ai_authority.py`, 전체 단위/회귀 테스트 100% 통과 |
