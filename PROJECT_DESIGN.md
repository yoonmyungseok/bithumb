# Bithumb & Upbit AI Pro Quant Trading Bot (v8.20)
...
- **손절 시 신규 매수 차단 완전 해제 (바닥 재매수 보장)**: 손절(STOP_LOSS, HARD_STOP, 손절 방어) 청산 발생 시에는 재진입 쿨다운 타이머를 적용하지 않으며(0초), 직전 손절가 대비 상방 돌파(+1.5%) 갭 필터 검사를 제거하여 손절 후 하락한 바닥 가격에서도 퀀트/AI 매수 승인 시 즉시 재매수가 가능하도록 허용합니다.
...
- **연속 손실 리스크 관리**: 연속 2회 이상 손절 시에도 신규 매수 쿨다운을 강제하지 않고, `get_risk_scale_factor()`(1회: 0.8, 2회 이상: 0.5)를 통한 자본 디스케일링만 적용하여 바닥 반등 매수 기회를 온전히 보존합니다.
...
- **Gemini AI 쿼터 및 캐시 절감 관측**: Google AI Studio 모델별 쿼터(Flash-Lite 2개 모델 교차 활용 기준 계정당 1,000 RPD, 통합 2,000 RPD) 대비 사용률 프로그레스 바, 5분봉 캐시 적중(쿼터 절감 횟수), 429 감지 및 로컬 퀀트 알고리즘 폴백 횟수를 대시보드에 실시간 제공합니다.
