...
- 전략의 추가·수정·삭제(진입·청산·필터·임계값·비중·레짐 경로 포함) 시에는 `src/gemini_analyzer.py`의 AI 매수 분석 명령 프롬프트를 반드시 함께 최신화한다. 프롬프트의 레짐·세션·후보 경로·승인 기준·안전 차단 조건·JSON 응답 스키마가 실제 전략과 일치하는지 검증하고, 프롬프트 회귀 테스트와 `PROJECT_DESIGN.md` 설명도 같은 변경에서 갱신한다.
